...
def parse_birth_year(artist_dates: str | None) -> int | None:
    if not artist_dates:
        return None
    match = re.search(r"\d{4}", artist_dates)
...
def main() -> None:
    ...
    OUTPUT_ARTISTS_JSON.parent.mkdir(parents=True, exist_ok=True)
    OUTPUT_ARTISTS_JSON.write_text(json.dumps(artist_wiki, indent=2, sort_keys=True), encoding="utf-8")

    OUTPUT_ARTISTS_JS.parent.mkdir(parents=True, exist_ok=True)
    artists_js_content = "// Generated by fetch_data.py — do not edit by hand.\n"
    artists_js_content += f"const ARTIST_WIKI = {json.dumps(artist_wiki, sort_keys=True)};\n"
